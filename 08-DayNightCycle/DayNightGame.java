...
/**
 * Lesson 8: Day/Night Cycle and Simple Lighting System
 * 
 * This builds on the HealthDamageGame and adds:
 * 1. Complete day/night cycle with realistic timing
 * 2. Dynamic sky color that changes throughout the day
 * 3. Simple lighting system with light sources
 * 4. Ambient light that decreases at night
 * 5. Light propagation and shadows
 * 6. Enhanced visual atmosphere
 * 7. Time-based enemy spawning (more enemies at night)
 */
...
enum TileType {
    ...
    TORCH(9, new Color(255, 200, 100), false, false, 14); // New: Torch block
...
class CelestialBodies {
    ...
    public CelestialBodies(long seed) {
        this.random = new Random(seed);
        ...
        // Generate clouds at random positions
        clouds = new Cloud[8];
    ...
    public void drawSun(Graphics2D g2d, TimeSystem timeSystem, int windowWidth, int windowHeight) {
        ...
        if (!timeSystem.isNight() && timeSystem.getTimeOfDay() > 0.2f && timeSystem.getTimeOfDay() < 0.8f) {
            // Calculate sun position (arc across sky)
            float dayProgress = (timeSystem.getTimeOfDay() - 0.2f) / 0.6f; // 0.0 to 1.0 during day
            
            int sunX = (int) (windowWidth * dayProgress);
            int sunY = (int) (windowHeight * 0.3 - Math.sin(dayProgress * Math.PI) * windowHeight * 0.2);
            
            // Draw sun glow
            for (int i = 3; i > 0; i--) {
                g2d.setColor(new Color(255, 255, 200, 20 * i));
                g2d.fillOval(sunX - 25 - i * 5, sunY - 25 - i * 5, 50 + i * 10, 50 + i * 10);
            }
            ...
            g2d.fillOval(sunX - 15, sunY - 15, 20, 20);
        }
    }
    ...
    public void drawMoon(Graphics2D g2d, TimeSystem timeSystem, int windowWidth, int windowHeight) {
        ...
        if (timeSystem.isNight()) {
            ...
            int moonX = (int) (windowWidth * nightProgress);
    ...
    public void drawClouds(Graphics2D g2d, TimeSystem timeSystem, int windowWidth) {
        // Draw clouds during day (more visible)
        if (!timeSystem.isNight()) {
            for (Cloud cloud : clouds) {
                cloud.draw(g2d, windowWidth, timeSystem.getAmbientLight());
            }
        }
    }
}
...
class Star {
    ...
    public Star(int x, int y, int size, float twinkleSpeed) {
        this.x = x;
        this.y = y;
        this.size = size;
    ...
    public void draw(Graphics2D g2d, float opacity) {
        twinkle += twinkleSpeed * 0.1f;
        float brightness = (float) (0.7f + Math.sin(twinkle) * 0.3f);
        
        int alpha = (int) (255 * opacity * brightness);
        g2d.setColor(new Color(255, 255, 255, Math.max(0, Math.min(255, alpha))));
        
        // Draw star as a cross pattern
        g2d.fillRect(x - size, y, size * 2 + 1, 1);
        g2d.fillRect(x, y - size, 1, size * 2 + 1);
...
class Cloud {
    ...
    public void draw(Graphics2D g2d, int windowWidth, float ambientLight) {
        // Cloud color changes with time of day
        int cloudBrightness = (int) (255 * ambientLight);
        g2d.setColor(new Color(cloudBrightness, cloudBrightness, cloudBrightness, 180));
        
        // Draw cloud as multiple overlapping circles
        int numCircles = 3;
        for (int i = 0; i < numCircles; i++) {
            int circleX = (int) x + (i * width / numCircles);
            int circleY = (int) y + (i % 2 == 0 ? 0 : height / 4);
            g2d.fillOval(circleX, circleY, width / 2, height);
        }
    }
}
...
class Player {
    ...
    public Player(float startX, float startY, World world) {
        ...
        // Give starting items including torches
...
public class DayNightGame extends JPanel implements Runnable, KeyListener, MouseListener {
    ...
    public DayNightGame() {
        ...
        celestialBodies = new CelestialBodies(seed);
        
        spawnEnemies();
        
        System.out.println("Day/Night Cycle and Lighting system initialized!");
        System.out.println("Controls: A/D - Move, Space - Jump, 1-9 - Select slot");
        System.out.println("Features: Dynamic day/night cycle, lighting system, time-based spawning");
        System.out.println("Place torches (item 3) to light up dark areas!");
    }
    ...
    private void update(long deltaTime) {
        ...
        // Update celestial bodies (clouds movement)
    ...
    protected void paintComponent(Graphics g) {
        ...
        // Draw celestial bodies (sun/moon, stars, clouds)
        ...
        celestialBodies.drawClouds(g2d, timeSystem, WINDOW_WIDTH);
        ...
        for (int x = startX; x < endX; x++) {
            for (int y = startY; y < endY; y++) {
                ...
                if (tile != TileType.AIR) {
                    ...
                    Color tileColor = lightingSystem.applyLighting(tile.getColor(), lightLevel, timeSystem.getAmbientLight());
    ...
    private void drawUI(Graphics2D g2d) {
        g2d.setColor(Color.WHITE);
        g2d.setFont(new Font("Arial", Font.BOLD, 12));
        g2d.drawString("Day/Night Cycle & Lighting Demo", 10, 20);
        ...
        g2d.drawString("Place torches to light up dark areas!", 10, 95);
    }
    ...
    public static void main(String[] args) {
        JFrame frame = new JFrame("Terraria Clone - Day/Night Cycle & Lighting");
        DayNightGame game = new DayNightGame();
